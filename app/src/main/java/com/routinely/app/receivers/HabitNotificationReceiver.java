...
public class HabitNotificationReceiver extends BroadcastReceiver {
    ...
    public static void schedule(Context ctx, Models.Habit habit) {
        ...
        for (int[] time : habit.getEffectiveReminderTimes()) {
            ...
            PendingIntent pi = buildPI(ctx, habit.id);
    ...
    /** Cancel all pending reminder alarms for a habit. */
    public static void cancel(Context ctx, int habitId) {
        AlarmManager am = (AlarmManager) ctx.getSystemService(Context.ALARM_SERVICE);
        if (am != null) am.cancel(buildPI(ctx, habitId));
    }
    ...
    private static PendingIntent buildPI(Context ctx, int habitId) {
        Intent i = new Intent(ctx, HabitNotificationReceiver.class);
        i.putExtra(EXTRA_HABIT_ID, habitId);
        int flags = PendingIntent.FLAG_UPDATE_CURRENT |
                (Build.VERSION.SDK_INT >= 23 ? PendingIntent.FLAG_IMMUTABLE : 0);
        return PendingIntent.getBroadcast(ctx, habitId + 10000, i, flags);
    }
}
